...
impl LaserScan {
    ...
    /// Add intensity data to this scan (builder pattern).
...
impl PointCloud2D {
    ...
    /// Add a point by x, y coordinates (convenience method).
    #[inline]
    pub fn push_xy(&mut self, x: f32, y: f32) {
        self.push(Point2D::new(x, y));
    }
...
mod tests {
    ...
    fn test_laser_scan_iter() {
        ...
        let mut scan = LaserScan::new(0.0, TAU, TAU / 3.0, 0.1, 10.0, ranges);
        scan.intensities = Some(vec![100, 150, 200]);
    ...
    fn test_point_cloud_2d_basic() {
        let mut cloud = PointCloud2D::with_capacity(0);
    ...
    fn test_point_cloud_2d_bounds() {
        let mut cloud = PointCloud2D::with_capacity(3);
        cloud.push(Point2D::new(-1.0, -2.0));
        cloud.push(Point2D::new(3.0, 4.0));
    ...
    fn test_point_cloud_2d_transform() {
        let mut cloud = PointCloud2D::with_capacity(1);
    ...
    fn test_empty_point_cloud_operations() {
        let cloud = PointCloud2D::default();
    ...
    fn test_point_cloud_clear() {
        let mut cloud = PointCloud2D::with_capacity(2);
        cloud.push(Point2D::new(1.0, 2.0));
        cloud.push(Point2D::new(3.0, 4.0));
