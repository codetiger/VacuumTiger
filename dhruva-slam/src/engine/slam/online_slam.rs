...
pub struct OnlineSlam {
    ...
    /// Scan-to-submap matcher (ICP only, for speed).
    submap_matcher: PointToPointIcp,
...
impl OnlineSlam {
    ...
    pub fn new(config: OnlineSlamConfig) -> Self {
        ...
        let submap_matcher = PointToPointIcp::new(IcpConfig {
            max_iterations: 20,
    ...
    fn needs_map_rebuild(&self) -> bool {
        // Rebuild if we have new finished submaps
    ...
    fn apply_scan_match(&mut self, match_result: &ScanMatchResult, odom_delta: &Pose2D) -> Pose2D {
        // Blend odometry prediction with scan match result
        // Weight by match quality
        let alpha = match_result.score.clamp(0.0, 1.0);
...
mod tests {
    ...
    fn create_test_scan() -> PointCloud2D {
        ...
        // Create a wall-like pattern
        for i in 0..100 {
            let x = i as f32 * 0.05;
            scan.push(Point2D::new(x, 2.0)); // Wall at y=2
        }
        // Add side walls
        for i in 0..40 {
            let y = i as f32 * 0.05;
            scan.push(Point2D::new(0.0, y));
            scan.push(Point2D::new(5.0, y));
        }
        scan
    }
    ...
    fn test_online_slam_creates_keyframes() {
        ...
        // First scan creates first keyframe
        slam.process_scan(&scan, &Pose2D::identity(), 0);
        assert_eq!(slam.keyframes().len(), 1);

        // Small motion - no new keyframe
        slam.process_scan(&scan, &Pose2D::new(0.1, 0.0, 0.0), 1000);
        assert_eq!(slam.keyframes().len(), 1);

        // Large motion - new keyframe
        slam.process_scan(&scan, &Pose2D::new(0.5, 0.0, 0.0), 2000);
        assert_eq!(slam.keyframes().len(), 2);
    }
