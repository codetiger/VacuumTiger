...
mod tests {
    ...
    #[test]
    fn test_to_point_cloud_with_offset_zero_offset() {
        // With zero offset, should produce same results as to_point_cloud
        let ranges = vec![1.0, 2.0, 3.0];
        let scan = LaserScan::new(0.0, PI, FRAC_PI_2, 0.1, 10.0, ranges);

        let cloud_no_offset = ScanConverter::to_point_cloud(&scan);
        let cloud_with_zero = ScanConverter::to_point_cloud_with_offset(&scan, 0.0);

        assert_eq!(cloud_no_offset.len(), cloud_with_zero.len());
        for i in 0..cloud_no_offset.len() {
            assert_relative_eq!(cloud_no_offset.xs[i], cloud_with_zero.xs[i], epsilon = 1e-6);
            assert_relative_eq!(cloud_no_offset.ys[i], cloud_with_zero.ys[i], epsilon = 1e-6);
        }
    }
