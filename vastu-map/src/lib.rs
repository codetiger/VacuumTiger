//! # Vastu-Map
//!
//! Occupancy grid mapping library for robot exploration and navigation.
//!
//! ## Overview
//!
//! Vastu-Map provides a multi-layer occupancy grid with semantic cell types:
//!
...
//! ## Features
//!
//! - **Map Exploration**: Frontier-based autonomous exploration
//! - **Path Finding**: A* on occupancy grid with safety margins
//! - **Coverage Planning**: Full surface sweeping (future)
//!
//! ## Quick Start
//!
//! ```rust,ignore
//! use vastu_map::{OccupancyGridMap, MapConfig, Pose2D, LidarScan};
//!
...
// Map queries (frontier detection, etc.)
pub mod query;
...
pub use core::{
    BumperSensors, Cell, CellType, CliffSensors, GridCoord, LidarScan, Pose2D, SensorObservation,
    WorldPoint,
};
...
impl ObserveResult {
    ...
    pub fn merge(&mut self, other: &ObserveResult) {
        ...
        self.frontiers_changed |= other.frontiers_changed;
    }
}
...
impl OccupancyGridMap {
    ...
    pub fn observe_lidar_with_matching(
        ...
        scan: &LidarScan,
        encoder_pose: Pose2D,
        matcher_config: &slam::CorrelativeMatcherConfig,
    ) -> (ObserveResult, Pose2D, slam::ScanMatchResult) {
        let matcher = slam::CorrelativeMatcher::new(matcher_config.clone());
