...
//!
//! VastuSLAM uses semantic cell types that distinguish between different
//! obstacle sources. This multi-sensor fusion approach allows the map to
//! represent obstacles that lidar cannot detect (glass, cliffs).
//!
//! ## Cell Type Hierarchy
//!
//! ```text
...
//! ├─────────┬──────────────────────────────────────────────────────┤
//! │  Bump   │ Physical collision - bumper triggered                 │
//! │         │ Examples: glass doors, mirrors, thin furniture legs   │
//! ├─────────┼──────────────────────────────────────────────────────┤
//! │  Cliff  │ Floor drop-off - cliff sensor triggered               │
//! │         │ Examples: stairs, ledges, thresholds                  │
//! ├─────────┼──────────────────────────────────────────────────────┤
//! │  Wall   │ Solid obstacle - lidar ray terminated                 │
//! │         │ Examples: walls, furniture, doors                     │
//! ├─────────┼──────────────────────────────────────────────────────┤
//! │  Floor  │ Traversable surface - no obstacles detected           │
//! │         │ Lidar rays pass through, safe for navigation          │
//! ├─────────┼──────────────────────────────────────────────────────┤
//! │ Unknown │ Not yet observed by any sensor                        │
//! │         │ Default state for unexplored areas                    │
//! └─────────┴──────────────────────────────────────────────────────┘
//! ```
//!
//! ## Priority Rules
//!
//! Higher-priority observations override lower-priority ones:
//! - `Bump` cannot be downgraded (physical evidence is definitive)
//! - `Cliff` can only be upgraded to `Bump`
//! - `Wall` can be upgraded to `Cliff` or `Bump`
//! - `Floor` can be upgraded to any obstacle type
//! - `Unknown` can be set to any type
