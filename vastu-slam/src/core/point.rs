...
//!
//! This module defines the two primary coordinate types used throughout VastuSLAM:
//!
//! - [`GridCoord`]: Integer cell indices for grid operations
//! - [`WorldPoint`]: Floating-point world coordinates in meters
//!
//! ## Coordinate Conversion
//!
//! Converting between world and grid coordinates requires knowing the grid's
//! origin and resolution. Use [`GridStorage::world_to_grid`] and
//! [`GridStorage::grid_to_world`] for conversion.
//!
//! ```text
...
///
/// Represents a discrete cell position in the occupancy grid. Grid coordinates
/// are integer indices where (0, 0) is the grid's internal origin (typically
/// not the world origin).
///
/// # Coordinate System
///
/// - **x**: Column index (increases rightward/forward in world)
/// - **y**: Row index (increases upward/leftward in world)
///
/// # Neighbor Access
///
/// Use [`neighbors_4`](Self::neighbors_4) for 4-connected (Manhattan) neighbors,
/// or [`neighbors_8`](Self::neighbors_8) for 8-connected (including diagonals).
///
/// # Example
///
/// ```rust,ignore
/// use vastu_slam::core::GridCoord;
///
/// let coord = GridCoord::new(5, 10);
/// let neighbors = coord.neighbors_4();  // N, E, S, W
///
/// // Manhattan distance for heuristics
/// let goal = GridCoord::new(10, 15);
/// let distance = coord.manhattan_distance(&goal);  // 10
/// ```
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, Default, Serialize, Deserialize)]
pub struct GridCoord {
    /// X coordinate (column index, increases in +X world direction)
    pub x: i32,
    /// Y coordinate (row index, increases in +Y world direction)
    pub y: i32,
}
...
/// World coordinates in meters (f32 precision).
///
/// Represents a point in the 2D world coordinate system. All measurements
/// are in meters, following the ROS REP-103 convention.
///
...
/// # Vector Operations
///
/// `WorldPoint` supports common vector operations:
/// - Addition/subtraction for displacement
...
///
/// ```rust,ignore
/// use vastu_slam::core::WorldPoint;
///
/// let origin = WorldPoint::ZERO;
/// let target = WorldPoint::new(3.0, 4.0);
///
/// // Distance calculation
/// let distance = origin.distance(&target);  // 5.0
///
/// // Angle from origin to target
/// let angle = origin.angle_to(&target);  // ~0.93 radians
///
/// // Create point at angle and distance from origin
/// let point = origin.point_at(angle, distance);  // (3.0, 4.0)
/// ```
#[derive(Clone, Copy, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct WorldPoint {
    /// X coordinate in meters (positive = forward in robot frame)
    pub x: f32,
    /// Y coordinate in meters (positive = left in robot frame)
    pub y: f32,
}
