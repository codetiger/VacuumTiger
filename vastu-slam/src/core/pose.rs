//! Robot pose representation and coordinate frame transformations.
//!
//! This module provides [`Pose2D`] for representing robot pose (position + orientation)
//! and utilities for coordinate frame transformations.
//!
//! ## Coordinate Frames
//!
//! VastuSLAM uses two primary coordinate frames:
//!
//! - **World Frame**: Fixed global coordinates (map origin)
//! - **Robot Frame**: Moves with the robot (robot is at origin, facing +X)
//!
//! ```text
...
//!
//! - [`Pose2D::transform_point`]: Robot frame → World frame
//! - [`Pose2D::inverse_transform_point`]: World frame → Robot frame
//! - [`Pose2D::compose`]: Combine two poses (chain transforms)
//! - [`Pose2D::inverse`]: Invert a pose transform

use super::point::WorldPoint;
use serde::{Deserialize, Serialize};

/// Robot pose in world coordinates (position + orientation).
///
/// Represents the robot's position (x, y) and heading (theta) in the world frame.
/// This is the fundamental type for tracking robot state and performing coordinate
/// transformations between robot and world frames.
///
...
///              -X (Back)
///
/// Theta = 0:     Robot facing +X (forward)
/// Theta = π/2:   Robot facing +Y (left)
/// Theta = π:     Robot facing -X (backward)
/// Theta = -π/2:  Robot facing -Y (right)
/// ```
///
/// # Example
///
...
///
/// // Robot at (1, 2) facing left (+Y direction)
/// let pose = Pose2D::new(1.0, 2.0, FRAC_PI_2);
///
/// // Point 1m ahead in robot frame
/// let sensor_point = WorldPoint::new(1.0, 0.0);
///
/// // Transform to world frame: 1m in +Y direction from robot
/// let world_point = pose.transform_point(sensor_point);
/// // Result: (1.0, 3.0)
/// ```
#[derive(Clone, Copy, Debug, Default, Serialize, Deserialize)]
pub struct Pose2D {
    /// X position in meters (world frame)
    pub x: f32,
    /// Y position in meters (world frame)
    pub y: f32,
    /// Orientation in radians, counter-clockwise from +X axis (range: [-π, π])
    pub theta: f32,
}
